...
const Navbar = ({ toggleSidebar, openModal }: NavbarProps) => {
  ...
  return (
    <nav className="sticky top-0 z-40 w-full backdrop-blur flex-none transition-colors duration-500 lg:z-50 lg:border-b lg:border-slate-900/10 dark:border-slate-50/[0.06] bg-white supports-backdrop-blur:bg-white/95 dark:bg-violeta5 shadow-lg fixed">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-4">
            <div className="hidden lg:flex space-x-4">
              ...
              <Link to="/CLibro" className={`text-white relative ${hoveredLink === 'Crear Libro' ? 'text-violeta5' : ''}`}onMouseEnter={() => handleMouseEnter('Crear Libro')} onMouseLeave={handleMouseLeave}>
                <div className={` p-5 transition-colors duration-300 ${hoveredLink === 'Crear Libro' ? 'bg-violeta7' : 'bg-transparent'}`}>Crear Libro</div>
              </Link>
            </div>
          </div>
        </div>
